...
const STATUSES = ['', 'pending', 'paid', 'rendering', 'ready', 'failed', 'refunded'];
...
export default async function ProjectsList({
  ...
  const rows = await adminApi.listProjects(params).catch(() => []);
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <form className="flex gap-2 text-sm">
          <select name="status" defaultValue={params.status ?? ''} className="rounded-md border-slate-300 px-2 py-1">
            {STATUSES.map((s) => (
              <option key={s || 'any'} value={s}>{s || 'Any status'}</option>
            ))}
          </select>
          <input
            type="search"
            ...
            placeholder="email or project id"
            ...
            {rows.map((p) => (
              <tr key={p.id} className="border-t border-slate-100 hover:bg-slate-50">
                <td className="px-4 py-2">
                  <Link href={`/projects/${p.id}`} className="font-mono text-xs text-sky-700 hover:underline">
                    {p.id}
                  </Link>
                </td>
                <td className="px-4 py-2">{p.customerEmail ?? '—'}</td>
                <td className="px-4 py-2">{p.templateId}</td>
                <td className="px-4 py-2">{p.arrangement}</td>

...
`;
  ...
  var NEUTRAL = 0.5;
  var IDEAL_BRIGHTNESS = 0.55;
  var SUPPORTING_MIN_SCORE = 40;
  var PHASH_DUP_DISTANCE = 8;
  var DUP_TIME_WINDOW_MS = 4e3;
  var clamp01 = (n) => n < 0 ? 0 : n > 1 ? 1 : n;
  var round = (n) => Math.round(n);
  function orientationOf(w, h2) {
    if (h2 <= 0 || w <= 0) return "square";
    const ratio = w / h2;
    if (ratio < 0.95) return "portrait";
    if (ratio > 1.05) return "landscape";
    return "square";
  }
  function hasSignal(n) {
  ...
  function inferOccasion(photos, explicit) {
  ...
  function recommendConcept(occasion, subject) {
    switch (occasion) {
      case "memorial":
      case "family_reunion":
      case "retirement":
  ...
  function scorePhoto(input, index) {
    const orientation = orientationOf(input.width, input.height);
    const megapixels = Math.max(0, input.width * input.height / 1e6);
    ...
    let faceComponent;
    if (!hasSignal(input.faceCount)) faceComponent = 0.4;
    else if (faceCount === 0) faceComponent = 0.2;
    else if (faceCount === 1) faceComponent = 1;
  ...
  function detectDuplicates(scored) {
    const groups = [];
    ...
    for (let i = 0; i < scored.length; i++) {
      ...
      for (let j = i + 1; j < scored.length; j++) {
        ...
        if (a.input.perceptualHash && b.input.perceptualHash) {
          ...
          if (Number.isFinite(dt) && dt <= DUP_TIME_WINDOW_MS && a.input.width === b.input.width && a.input.height === b.input.height) {
            dup = true;
  ...
  function detectRestoration(scored) {
    const out = [];
  ...
  function aggregateColors(scored) {
    ...
    const colors = [...weights.entries()].map(([hex, w]) => ({ hex, weight: w / total })).sort((a, b) => b.weight - a.weight).slice(0, 6).map((c) => ({ hex: c.hex, weight: round(c.weight * 100) / 100 }));
    return { colors, hadData: true };
  }
  function buildGroups(scored) {
    const byDay = /* @__PURE__ */ new Map();
    for (const s of scored) {
      if (!s.input.takenAt) continue;
      const d = new Date(s.input.takenAt);
      if (Number.isNaN(d.getTime())) continue;
      const label2 = d.toISOString().slice(0, 10);
      if (!byDay.has(label2)) byDay.set(label2, []);
      byDay.get(label2).push(s.input.id);
    }
    return [...byDay.entries()].filter(([, ids]) => ids.length > 0).sort((a, b) => a[0] < b[0] ? -1 : 1).map(([label2, photoIds]) => ({ label: label2, photoIds, size: photoIds.length }));
  }
  function subjectFromFaces(faceCount, sourcePhotoId) {
    let type;
  ...
  function supportingCap(occasion) {
    switch (occasion) {
      case "championship":
      case "social":
        return 5;
  ...
  function generateMemoryProfile(uploadedPhotos, options = {}) {
    ...
    const isSuppressedDuplicate = (id) => dupAll.has(id) && !dupKeep.has(id);
    const heroPool = [...scored].filter((s) => !isSuppressedDuplicate(s.input.id));
    const heroSorted = [...heroPool].sort(
      (a, b) => b.heroScore - a.heroScore || b.quality - a.quality || b.megapixels - a.megapixels || b.faceCount - a.faceCount || a.index - b.index
    );
    const hero = heroSorted[0] ?? null;
    const cap = supportingCap(occasion);
    const supportingSorted = scored.filter((s) => s !== hero && !isSuppressedDuplicate(s.input.id)).sort((a, b) => b.supportScore - a.supportScore || a.index - b.index);
    const supporting = supportingSorted.filter((s) => s.supportScore >= SUPPORTING_MIN_SCORE).slice(0, cap);
    const supportingIds = new Set(supporting.map((s) => s.input.id));
    const roleOf = (s) => s === hero ? "hero" : supportingIds.has(s.input.id) ? "supporting" : "excluded";
    const photo_rankings = [...scored].sort(
      (a, b) => (a === hero ? 1e9 : a.supportScore) < (b === hero ? 1e9 : b.supportScore) ? 1 : -1
    ).map((s, i) => ({
      photoId: s.input.id,
      rank: i + 1,
    ...
    const photo_quality_scores = scored.map((s) => ({
      photoId: s.input.id,
      quality: s.quality,
    ...
    const avgBrightness = scored.reduce((sum, s) => sum + (hasSignal(s.input.brightness) ? s.input.brightness : NEUTRAL), 0) / scored.length;
    const baseMood = MOOD_BY_OCCASION[occasion];
    const mood = anyAnalysisSignal && avgBrightness < 0.4 ? `${baseMood} \xB7 dramatic` : baseMood;
    const { colors, hadData: hadColor } = aggregateColors(scored);
    const dominant_colors = hadColor ? colors : DEFAULT_PALETTE[occasion].map((hex, i, arr) => ({
      hex,
      weight: round(100 * (arr.length - i) / (arr.length * (arr.length + 1) / 2)) / 100
    }));
    ...
    if (!anyAnalysisSignal) {
      warnings.push({
        ...
        message: "No per-photo analysis signals (sharpness/faces/brightness) supplied; scores use neutral defaults.",
        severity: "info"
      });
    }
    ...
    else if (!options.occasion) confidence -= 8;
  ...
  function baseFractions(profile, brief) {
    ...
    const dom = clamp012(brief.heroStrategy.dominanceRatio || 0.5);
    const suppCount = profile.supporting_photos.length;
    const groups = profile.groups.length;
    const conf = clamp012((profile.confidence_score + brief.confidenceScore) / 2 / 100);
    ...
    return {
      heroStrength: clamp012(heroScore * 0.85 + dom * 0.15),
      emotionalImpact: clamp012((0.58 + heroScore * 0.2 + emoKeywords * 0.15 + (heroFaces > 0 ? 0.1 : 0) + (emotionalOccasion ? 0.04 : 0)) * confFactor),
      storytelling: clamp012(0.6 + Math.min(suppCount, 5) / 5 * 0.3 + Math.min(groups, 3) / 3 * 0.14),
      layoutBalance: clamp012((0.76 + dom * 0.08 + (suppCount > 0 ? 0.08 : 0) + (whitespaceGenerous ? 0.06 : 0) - (overcrowd ? 0.12 : 0)) * confFactor),
      typography: clamp012(0.85 + (brief.typographyDirection ? 0.08 : 0)),
      colorHarmony: clamp012((colorFromPhotos ? 0.9 : 0.82) + 0.05),
      luxuryFinish: clamp012(0.8 + (whitespaceGenerous ? 0.04 : 0)) * confFactor,
      shareability: clamp012(0.76 + (socialOccasion ? 0.14 : 0.08) + heroScore * 0.06)
    };
  }
  function scoreConcept(concept, profile, brief) {
    ...
    let totalRaw = 0;
    for (const c of cats) {
      const frac = clamp012(base[c] * mod[c]);
      const p = frac * MAX[c];
      points[c] = round1(p);
      totalRaw += p;
    }
  ...
  function heroDominance(concept, brief) {
    const base = clamp013(brief.heroStrategy.dominanceRatio || 0.55);
    switch (concept) {
      case "Luxury Gold":
  ...
  function directArt(memoryProfile, creativeBrief, wowPresentation, options = {}) {
    ...
    return { schemaVersion: SCHEMA_VERSION6, occasion, directions, presentation };
  }
  ...
  // components/integration/wow-bridge.ts
  var THEME_OCCASION = {
    graduation: "graduation",
  ...
  function selectConcept(state, conceptName) {
    state.wowSelectedConcept = conceptName;
  }

  // components/integration/wow-bridge.entry.ts
  ...
  function sampleSignals(el) {
    try {
      ...
      const clamp2 = (v) => v < 0 ? 0 : v > 1 ? 1 : v;
      return { brightness: clamp2(mean / 255), contrast: clamp2(std / 64), sharpness: clamp2((n ? edge / n : 0) / 40) };
    } catch {
      return {};
    }
  }
  var enrichFromUpload = (photo) => photo.imgEl ? sampleSignals(photo.imgEl) : {};
  ...
  async function showReveal(state, slot, handlers = {}) {
    try {
      ...
      const directions = new Map(pipeline.artDirection.directions.map((d) => [d.conceptName, d]));
      ...
      mountPremiumReveal(slot, {
        ...
        handlers: {
          onChoose: (c) => {
            selectConcept(state, c.conceptName);
            handlers.onSelect?.(c.conceptName);
          },
          onDetails: (c) => {
            handlers.onDetails?.(c.conceptName);
          }
        }
      });

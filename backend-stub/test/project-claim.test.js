 ...
 *
 * A project is unclaimed until it has been SAVED (render_input IS NULL). Once a design
 * exists a valid token is required, so the trust-on-first-use window is unchanged.
 */
...
stub('db/projects', {
  ...
  createIfMissing: async () => {
    if (!state.row) state.row = { id: 'proj_test01', render_input: null };
  },
  saveRenderInput: async ({ projectId, renderInput, rev }) => {
    state.saved = { projectId, renderInput, rev };
    state.row = { id: projectId, render_input: renderInput };
    return rev + 1;
  },
...
test('THE DEFECT: a row pre-created by the upload endpoint can still be claimed', async () => {
  ...
  const r = await call({ row: { id: PID, render_input: null } });
...
test('once a design exists, an anonymous write is refused', async () => {
  const r = await call({ row: { id: PID, render_input: { theme: { id: 'graduation' } } } });
...
test('the owner’s token still authorizes writes to a saved project', async () => {
  const r = await call({
    row: { id: PID, render_input: { theme: { id: 'graduation' } } },
...
test('render_input of empty-string/0 is still treated as unsaved, not as a claim', async () => {
  for (const empty of [null, undefined]) {
    const r = await call({ row: { id: PID, render_input: empty } });
    assert.equal(r.status, 200, `render_input=${String(empty)} must remain claimable`);
  }
});
